...
const { getAppName, getAppNameSlug } = require('../utils/appBrand');
...
async function fetchLogoBuffer(url) {
  ...
  try {
    const res = await axios.get(u, {
      ...
      headers: { Accept: 'image/*,*/*', 'User-Agent': `${getAppNameSlug()}-Label/1.0` },
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (s.misc?.showPoweredBy) {
        txt(doc, `Powered By ${vm.poweredBy || getAppName()}`, x0 + innerW - 108, footY, 102, {
          fontSize: 5.2,
          align: 'right',
          height: 8
        });
  ...
  <div class="body-tail">
    ...
    <div class="foot">
      <div class="foot-row">
        ...
        <span class="b">${s.misc?.showPoweredBy ? escapeHtml('Powered By ' + (vm.poweredBy || getAppName())) : ''}</span>
      </div>
    </div>
  </div>
</div>
